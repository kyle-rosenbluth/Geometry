...
public class MeshGenerator : MonoBehaviour {
	...
	void Start () {
		//createTaurus(1.0f,5.0f,50,50);
		//createTetrahedron();
		//createCube();
		//createSphere(1,100,100);
		//createCylinder(1.0f,5.0f,8, gameObject);
		//createWireframe();
		//drawCylinderWithEndpoints(new Vector3(0,0,0), new Vector3(1,1,0), 1);
		drawCylinderWithEndpoints(new Vector3(1,0,0), new Vector3(5,0,0), 1);
		
	}
	
	void drawCylinderWithEndpoints(Vector3 startPoint, Vector3 endPoint, int cylNum) {
	
		Vector3 legStartPoint = new Vector3(endPoint.x, startPoint.y);
		Vector3 legEndPoint = endPoint;
		float legDistance = Mathf.Sqrt(Mathf.Pow((legEndPoint.x - legStartPoint.x), 2) + Mathf.Pow((legEndPoint.y - legStartPoint.y), 2));
		Debug.Log("leg dist" + legDistance);
		float hypDistance = Mathf.Sqrt(Mathf.Pow((endPoint.x - startPoint.x), 2) + Mathf.Pow((endPoint.y - startPoint.y), 2));
		Debug.Log("hyp Dist" + hypDistance);
		
		float angle = Mathf.Asin(legDistance / hypDistance);
		Debug.Log(angle * (180.0f / Mathf.PI));
		GameObject cyl = GameObject.Find(string.Format("Cylinder{0}", cylNum));
		createCylinder(0.1f, hypDistance, 8,cyl);
		...
		cyl.transform.Translate(startPoint);
		cyl.transform.Rotate(new Vector3(0,0,1), angle * (180.0f / Mathf.PI));
...
//			Debug.DrawLine(vertices[i], (new Vector3(.2f,.2f,.2f)) + vertices[i], Color.red);
//		}
//	}
	void createTriangle() {
...
//CYLINDER	
	public void createCylinder(float radius, float height, int slices, GameObject go) {
		...
		createCylinder(radius, height, slices, go, identity);
	}
